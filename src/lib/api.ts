...
const BASE_URL = import.meta.env.VITE_API_URL || "http://api.8.231.116.253.nip.io";
...
export const api = {
  ...
  if (!response.ok) {
    const error = await response.text();
    console.error("Failed file content:", error);
...
async createProject(name: string): Promise<ProjectSummaryResponse | null> {
  const response = await fetch(`${BASE_URL}/api/v1/workspace/projects`, {
    method: "POST",
